...
- Входные данные кроме номера карты генерируются при помощи библиотеки Faker. Имя владельца. CVV код - 3 цифры. Год окончания действия карты - не более 5 лет с текущей даты. Месяц - номер месяца состоит из двух цифр. Номер карты: 4444 4444 4444 4441.
...
- В поле "год" вводится значение "1".
- Остальные входные данные кроме месяца генерируются при помощи библиотеки Faker.
- Ожидаемый результат: Сообщение "неверный формат" под полем "месяц".
